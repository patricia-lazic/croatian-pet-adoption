  ...
  <div class="space-y-4">
    <div>
      ...
      <p class="text-xs text-gray-500 mb-4">
        You can either upload an image from your computer or provide an image URL
      </p>
    </div>
      ...
      <div v-if="selectedFiles.length > 0" class="space-y-2">
        ...
        <div class="border rounded-lg p-2 max-w-xs">
          ...
          <div class="flex justify-between items-center mt-2">
            <p class="text-xs text-gray-600 truncate">{{ selectedFiles[0].name }}</p>
            <button
              type="button"
              @click="removeFile"
          ...
          <div v-if="selectedFiles[0].uploading" class="mt-2">
            ...
            <p class="text-xs text-gray-500 mt-1">{{ selectedFiles[0].progress }}%</p>
          </div>
...
const currentImageUrls = computed(() => {
  if (uploadMethod.value === 'file') {
    ...
    const uploadedFile = selectedFiles.value.find(file => file.uploaded && file.url)
    return uploadedFile ? [uploadedFile.url] : []
  } else {

    return singleUrl.value && singleUrl.value.trim() !== '' ? [singleUrl.value.trim()] : []
  }
})
...
const handleFileSelect = (event) => {
  ...
  if (file.size > props.maxFileSize) {
    error.value = `File ${file.name} is too large. Maximum size is ${props.maxFileSize / (1024 * 1024)}MB`
    return
  }
...
const uploadFiles = async () => {
  ...
  try {
    ...
    if (err.code === 'storage/unauthorized') {
      error.value = 'Upload permission denied. Please make sure you are logged in and have proper permissions.'
    } else {
