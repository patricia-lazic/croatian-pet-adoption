    ...
    >
      <div class="p-6">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              >
                ...
                <option value="Yes">Yes</option>
                <option value="No">No</option>
                <option value="With proper introduction">
          ...
          <!-- Adoption Status - Only show when editing -->
          <div v-if="isEditing">
            <label class="block text-sm font-medium text-gray-700 mb-2"
              >Adoption Status</label
            >
            <select
              v-model="dogForm.isAdopted"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-pink-500 focus:border-transparent"
            >
              <option :value="false">Available for Adoption</option>
              <option :value="true">Adopted</option>
            </select>
            <p v-if="dogForm.isAdopted" class="text-sm text-gray-600 mt-1">
              Note: Marking as adopted will remove this dog from public
              listings.
            </p>
          </div>
  ...
  (newDog) => {
    if (newDog) {
      dogForm.value = {
        ...
        isAdopted: newDog.isAdopted || false,
      };
    } else {
      resetForm();
    }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      success.value = dogData.isAdopted
        ? "Dog marked as adopted successfully!"
