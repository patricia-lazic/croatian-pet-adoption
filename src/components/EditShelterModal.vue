  ...
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-8 max-w-lg w-full mx-4 max-h-screen overflow-y-auto">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">🏢 Edit Shelter Information</h2>
      ...
      <form @submit.prevent="handleSubmit">
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Shelter Name *</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">About Your Shelter</label>
            <textarea
...
const handleSubmit = async () => {
  ...
  try {
    const shelterData = {
      name: shelterForm.value.name,
      address: shelterForm.value.address,
      phone: shelterForm.value.phone,
      email: shelterForm.value.email,
      workingHours: shelterForm.value.workingHours || 'Contact for hours',
      description: shelterForm.value.description,
      website: shelterForm.value.website,
      location: extractLocationFromAddress(shelterForm.value.address)
    }
    ...
    error.value = 'Failed to update shelter information. Please try again.'
  } finally {
